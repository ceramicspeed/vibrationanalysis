...
################################################################################
    ...
    def PerformAnalysis(self):
        ...
        if(n==m):
            from vb_sine_tone_gui import vb_sine_tone        
            vb_sine_tone(win)
            
        m=m+1        

        if(n==m):
            from vb_white_noise_sound_gui import vb_white_noise_sound       
            vb_white_noise_sound(win)
            
        m=m+1        

        if(n==m):
            from vb_Shepard_Tone_gui import vb_Shepard_Tone        
            vb_Shepard_Tone(win)     
  
        m=m+1        

        if(n==m):
            from vb_read_wav_gui import vb_read_wav        
            vb_read_wav(win)  
