...
class vb_classical_pulse_base:
    ...
    def __init__(self,parent): 
        ...
        self.master.title("vb_classical_base_gui.py ver 1.3  by Tom Irvine")         
...
################################################################################
    ...
    def export_acceleration(self):
        ...
        if(self.n_at==0):
            WriteData2(self.nt[0],self.tt,self.acc,output_file)        
        else:
            nf=len(self.fn)
            WriteData3(nf,self.fn,self.amax,abs(self.amin),output_file)
...
################################################################################
    ...
    def export_rel_disp(self):
        ...
        if(self.n_at==0):
            WriteData2(self.nt[0],self.tt,self.rd,output_file)        
        else:
            nf=len(self.fn)
            WriteData3(nf,self.fn,self.rdmax,abs(self.rdmin),output_file)
...
##        
        n_scale= int(self.Lb_scale.curselection()[0])
        
        if(n_scale==0):
            self.rd_scale=386.
        else:
            self.rd_scale=9.81*1000.
...
##
        ...
        if(self.n_at==0):
            ...
            vb_classical_pulse_base.classical_plot_th(self.Q,f_string,\
                                      self.tt,self.acc,self.yb,self.rd,n_scale)
...
#  Find residual parameters
            ...
            self.rd*=self.rd_scale                
    
            self.amax[k]=max(self.acc)
            self.amin[k]=min(self.acc)

            self.rdmax[k]=max(self.rd)
            self.rdmin[k]=min(self.rd)
